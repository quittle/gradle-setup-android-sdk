 ...
 *
 * === FORK ALERT ===
 * This is mostly a fork of the Android Gradle's
 * {@code com.android.sdklib.repository.FullRevision} class
 */
@SuppressWarnings({"allJavadoc", "PMD"})
@SuppressFBWarnings
public class AndroidRevision implements Comparable<AndroidRevision> {
    ...
    public static final AndroidRevision NOT_SPECIFIED = new AndroidRevision(MISSING_MAJOR_REV);
    ...
    public AndroidRevision(int major) {
        this(major, IMPLICIT_MINOR_REV, IMPLICIT_MICRO_REV);
    }

    public AndroidRevision(int major, int minor, int micro) {
        this(major, minor, micro, NOT_A_PREVIEW);
    }

    public AndroidRevision(int major, int minor, int micro, int preview) {
        this(major, minor, micro, preview, " ");
    }

    public AndroidRevision(int major, int minor, int micro, int preview, String previewSeparator) {
        mMajor = major;
        mMinor = minor;
        mMicro = micro;
        mPreview = preview;
     ...
     * a new {@link AndroidRevision} for it. All the fields except major are
     * optional.
     * <p/>
     ...
     * @return A new non-null {@link AndroidRevision}.
     * @throws NumberFormatException if the parsing failed.
     */
    public static AndroidRevision parseRevision(String revision)
            throws NumberFormatException {
        return parseRevisionImpl(revision, true /*supportMinorMicro*/, true /*supportPreview*/);
    }

    protected static AndroidRevision parseRevisionImpl(String revision,
        ...
        try {
            ...
            if (m != null && m.matches()) {
                ...
                if (error == null) {
                    return new AndroidRevision(major, minor, micro, preview, previewSeparator);
                }
            }
        } catch (Throwable t) {
            cause = t;
    ...
    public boolean equals(Object rhs) {
        ...
        if (!(rhs instanceof AndroidRevision)) {
            return false;
        }
        AndroidRevision other = (AndroidRevision) rhs;
     ...
     *
     * @param rhs The right-hand side {@link AndroidRevision} to compare with.
     * @return <0 if lhs < rhs; 0 if lhs==rhs; >0 if lhs > rhs.
     */
    @Override
    public int compareTo(AndroidRevision rhs) {
     ...
     * @return <0 if lhs < rhs; 0 if lhs==rhs; >0 if lhs > rhs.
     */
    public int compareTo(AndroidRevision rhs, PreviewComparison comparePreview) {
     ...
     *  {@link AndroidRevision#compareTo(AndroidRevision, PreviewComparison)} */
